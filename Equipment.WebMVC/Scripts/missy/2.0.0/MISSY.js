...
; !function (window, undefined) {
    ...
    window.MISSY = {
                ...
                if (IdArray)                    
                    for (var i = 0, max = IdArray.length; i < max; i++) {
                        ...
                        if (node){
                            ...
                            if (flag === true)
                                this.zTreeCheckParentNodes(treeObj, node.Pid);
                        }
                    }
            }
        }
        , zTreeCheckParentNodes: function (treeObj, Id) {
            if (treeObj) {
        ...
        ,ShowWin: function (url, title, width, height) {
        var dv = $("<div class=\"ModalDialog\"></div>");
        dv.append("<div style=\"filter: alpha(Opacity=50); -moz-opacity: 0.5; opacity: 0.5; position: fixed;z-index: 99999; background-color: Black;top:0;left:0;width:100%;height:100%;\"></div>");
        var content = $("<div style=\"position: fixed;z-index: 100000;width:" + width + "px;height:" + (height + 40) + "px;background-color:#f0f2f9\"></div>").appendTo(dv);
        content.append("<h2 style=\"background-color: #dfecfa; font-size: 16px; padding: 0px 10px; line-height: 40px;\">" + title + "<span class=\"win_close\" style=\"float:right;cursor: pointer;\">X</span></h2>");
        content.append("<iframe width=\"100%\" height=\"" + height + "\" frameborder=\"0\" src=\"" + url + "\" scrolling=\"No\"></iframe>");
        var left = ($(window).width() - width) / 2;
        var top = ($(window).height() - height - 40) / 2;
        content.css({ "top": top + "px", "left": left + "px" });
        content.find(".win_close").on("click", function () {
            var win = $("#mainFrame").length > 0 ? $("#mainFrame")[0].contentWindow : window;
            if (typeof win.ClickRefreshList === "function")
                win.ClickRefreshList();
